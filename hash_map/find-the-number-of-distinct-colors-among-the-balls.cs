/*
  ...
  There are limit + 1 balls with distinct labels in the range [0, limit]. Initially, all balls are uncolored. For every query in queries that is of the form [x, y], 
...
public class Solution {
    public int[] QueryResults(int limit, int[][] queries) {
        ...
        for (int i = 0; i < queries.Length; ++i) {
            ...
            if (balls.TryGetValue(b, out int col) && --colors[col] == 0) {
                colors.Remove(col);
            }
