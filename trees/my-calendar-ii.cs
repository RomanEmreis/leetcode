...
public class MyCalendarTwo {
    ...
    public bool Book(int start, int end) {
        foreach (var (os, oe) in overlaps) {
            if (Math.Max(os, start) < Math.Min(oe, end)) {
                return false;
            }
        }

        foreach (var (bs, be) in books) {
            int overlappingStart = Math.Max(bs, start);
            int overlappingEnd = Math.Min(be, end);
            if (overlappingStart < overlappingEnd) {
                overlaps.Add((overlappingStart, overlappingEnd));
            }
        }
