/*
  ...
  There exist n rectangles in a 2D plane with edges parallel to the x and y axis. You are given two 2D integer arrays bottomLeft and topRight 
  ...
  Output: 1
  Explanation:
  A square with side length 1 can fit inside either the intersecting region of rectangles 0 and 1 or the intersecting region of rectangles 1 and 2. 
  Hence the maximum area is 1. It can be shown that a square with a greater side length can not fit inside any intersecting region of two rectangles.
...
impl Solution {
    pub fn largest_square_area(bottom_left: Vec<Vec<i32>>, top_right: Vec<Vec<i32>>) -> i64 {
        ...
        for i in 0..n {
            for j in i + 1..n {
                let ax1 = bottom_left[i][0];
                let ay1 = bottom_left[i][1];

                let ax2 = top_right[i][0];
                let ay2 = top_right[i][1];

                let bx1 = bottom_left[j][0];
                let by1 = bottom_left[j][1];

                let bx2 = top_right[j][0];
                let by2 = top_right[j][1];

                let overlap_x = ax2.min(bx2) - ax1.max(bx1);
                let overlap_y = ay2.min(by2) - ay1.max(by1);

                res = res.max(overlap_x.min(overlap_y));
            }
        }
