/*
  ...
  2	54	58	78	 86	102
  3	86	88	98	102	110
  As an example for why wizard 0 cannot start working on the 1st potion before time t = 52, consider the case where the wizards started preparing the 1st potion at time t = 50. At time t = 58,
...
impl Solution {
    pub fn min_time(skill: Vec<i32>, mana: Vec<i32>) -> i64 {
        let skill = skill.into_iter().map(|s| s as i64).collect::<Vec<_>>();
        let mana = mana.into_iter().map(|m| m as i64).collect::<Vec<_>>();
        let sum_skill: i64 = skill.iter().sum();
        let mut res = sum_skill * mana[0];
        let n = skill.len();
        for j in 1..mana.len() {
            let mut curr = res;
            if n > 1 {
                for i in (0..=n - 2).rev() {
                    curr -= skill[i + 1] * mana[j - 1];
                    res = curr.max(res - skill[i] * mana[j]);
                }
            }
            res += sum_skill * mana[j];
        }        
        res
    }
}
