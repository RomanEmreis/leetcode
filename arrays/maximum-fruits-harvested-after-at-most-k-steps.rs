/*
  ...
  It takes one step to move one unit on the x-axis, and you can walk at most k steps in total. For every position you reach, you harvest all the fruits at that position, and the fruits will disappear from that position.
...
impl Solution {
    pub fn max_total_fruits(fruits: Vec<Vec<i32>>, start_pos: i32, k: i32) -> i32 {
        ...
        for j in 0..fruits.len() {
            ...
            prefix += n;
            while i <= j && 
                  p - fruits[i][0] + (start_pos - fruits[i][0]).abs().min((start_pos - p).abs()) > k {
                prefix -= fruits[i][1];
                i += 1;
            }
            result = result.max(prefix);
        }
        result
    }
}
