        ...
        for (int command : commands) {
            if (command == -1) direction = (direction + 1) % 4;
                ...
                for (size_t i = 0; i < command; ++i) {
                    int next_x = pos.first + directions[direction].first;
                    int next_y = pos.second + directions[direction].second;

                    pair<int, int> next_pos = {next_x, next_y};
                    if (obstacles_set.contains(next_pos)) break;

                    pos = next_pos;

                    int dist = pos.first * pos.first + pos.second * pos.second;
