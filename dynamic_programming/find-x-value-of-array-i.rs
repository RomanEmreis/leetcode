...
impl Solution {
    pub fn result_array(nums: Vec<i32>, k: i32) -> Vec<i64> {
        ...
        for &value in &nums {
            ...
            let value_mod = value as usize % k;
            next[value_mod] = 1;

            for remainder in 0..k {
                let count = ending_here[remainder];
                ...
                let new_remainder =
                    remainder * value_mod % k;

                next[new_remainder] += count;
            }
            ...
            std::mem::swap(&mut ending_here, &mut next);
        }
