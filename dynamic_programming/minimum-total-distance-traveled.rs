...
impl Solution {
    pub fn minimum_total_distance(mut robot: Vec<i32>, mut factory: Vec<Vec<i32>>) -> i64 {
        ...
        let n = robot.len();
        let m = factory.len();
        let mut dp = vec![vec![0_i64; m]; n];

        dfs(0, 0, n, m, &robot, &factory, &mut dp)
    }
}
...
fn dfs(
    ...
    let mut res = dfs(i, j + 1, n, m, r, f, dp);
    let mut t = 0;
    let l = f[j][1] as usize;
    for k in 0..l {
        if i + k >= n {
            break;
        }
        t += (r[i + k] - f[j][0]).abs() as i64;
        res = res.min(t + dfs(i + k + 1, j + 1, n, m, r, f, dp));
    }
