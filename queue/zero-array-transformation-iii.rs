...
impl Solution {
    pub fn max_removal(nums: Vec<i32>, mut queries: Vec<Vec<i32>>) -> i32 {
        ...
        for (i, num) in nums.into_iter().enumerate() {
            while query_index < queries.len() && queries[query_index][0] as usize <= i {
                available.push(queries[query_index][1]);
                query_index += 1;
            }

            while let Some(&Reverse(top)) = running.peek() {
                if top < i as i32 {
                    running.pop();
                } else {
                    break;
            ...
            while num > running.len() as i32 {
                match available.peek() {
                    Some(&r) if r >= i as i32 => {
                        running.push(Reverse(r));
                        available.pop();
                    }
                    _ => return -1,
                }
