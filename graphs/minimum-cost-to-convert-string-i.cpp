...
class Solution {
private:
    const short alphabet = 26;
    const long long max_cost = 100000 * alphabet;

public:
    long long minimumCost(string source, string target, vector<char>& original, vector<char>& changed, vector<int>& cost) {
        vector<vector<long long>> matrix(alphabet, vector<long long>(alphabet, max_cost));
        ...
        for (int k = 0; k < n; ++k) {
            ...
            matrix[i][j] = min(matrix[i][j], (long long)cost[k]);
        }
        ...
        n = source.size();
        for (int i = 0; i < n; ++i) {
            int c = matrix[source[i] - 'a'][target[i] - 'a'];
