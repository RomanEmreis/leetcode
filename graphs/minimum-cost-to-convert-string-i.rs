...
impl Solution {
    pub fn minimum_cost(source: String, target: String, original: Vec<char>, changed: Vec<char>, cost: Vec<i32>) -> i64 {
        ...
        for k in 0..original.len() {
            let i = (original[k] as u8 - b'a') as usize;
            let j = (changed[k] as u8 - b'a') as usize;
            mat[i][j] = mat[i][j].min(cost[k] as i64);
        }
        ...
        for k in 0..26 {
            for i in 0..26 {
                for j in 0..26 {
                    mat[i][j] = mat[i][j].min(
                        mat[i][k].saturating_add(mat[k][j])
                    );
                }
            }
        }
        ...
        for k in 0..s.len() {
            let i = (s[k] - b'a') as usize;
            let j = (t[k] - b'a') as usize;
            let c = mat[i][j];
            if c == i64::MAX {
                return -1;
            }
